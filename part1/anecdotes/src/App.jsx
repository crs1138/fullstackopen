import { useState } from "react";
import Anecdote from "./Anecdote";
import Button from "./Button";

const App = () => {
  const anecdotes = [
    "If it hurts, do it more often.",
    "Adding manpower to a late software project makes it later!",
    "The first 90 percent of the code accounts for the first 10 percent of the development time...The remaining 10 percent of the code accounts for the other 90 percent of the development time.",
    "Any fool can write code that a computer can understand. Good programmers write code that humans can understand.",
    "Premature optimization is the root of all evil.",
    "Debugging is twice as hard as writing the code in the first place. Therefore, if you write the code as cleverly as possible, you are, by definition, not smart enough to debug it.",
    "Programming without an extremely heavy use of console.log is same as if a doctor would refuse to use x-rays or blood tests when diagnosing patients.",
  ];

  const [selected, setSelected] = useState(0);
  const [points, setPoints] = useState(Array.from(anecdotes, () => 0));

  const getRandomIndex = (myArray) => {
    return Math.floor(Math.random() * (myArray.length - 1));
  };

  const handleVote = () => {
    const pointsCopy = [...points];
    pointsCopy[selected] += 1;
    setPoints(pointsCopy);
  };

  const handleClick = () => {
    const anecdoteIndex = getRandomIndex(anecdotes);
    setSelected(anecdoteIndex);
  };

  const getMostVotesIndex = (points) => {
    const { index: mostVotesindex } = points.reduce(
      (higest, next, index) => {
        return next - higest.value > 0 ? { index, value: next } : higest;
      },
      { index: 0, value: 0 }
    );
    return mostVotesindex;
  };

  return (
    <div>
      <h2>Anecdote of the day</h2>
      <Anecdote>{anecdotes[selected]}</Anecdote>
      <p>has {points[selected]} votes</p>
      <Button handleClick={handleVote}>Vote</Button>
      <Button handleClick={handleClick}>Next anecdote</Button>
      <h2>Anecdote with most votes</h2>
      <Anecdote>{anecdotes[getMostVotesIndex(points)]}</Anecdote>
    </div>
  );
};

export default App;
